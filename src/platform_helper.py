    ...
    if chinese_parts:
        ...
        for suffix in suffix_patterns:
            if search_keyword.endswith(suffix):
                ...
                break
        if "的" in search_keyword and search_keyword.index("的") >= 3:
            search_keyword = search_keyword[: search_keyword.index("的")]
        if len(search_keyword) > 8:
            search_keyword = search_keyword[:5]
        elif len(search_keyword) > 6:
            search_keyword = search_keyword[:4]
    else:
        search_keyword = cleaned_title.replace("×", " ").strip()
